...
def main():
    ...
    parent_object_base = sw.load_parent_bone_inversely(pw, "camera_front_beam")
    name_parent = pw.fetch("camera_front_beam", BoneKeys.parent)
    sw.parent(parent_object_base).load_stl(os.path.join(PathInfo.dir_parts_renamed, name_parent + ".stl")).name = name_parent

    pw.remove_rotation("camera_front_beam")
    objects = sw.load_bones(pw, "camera_front_beam")
